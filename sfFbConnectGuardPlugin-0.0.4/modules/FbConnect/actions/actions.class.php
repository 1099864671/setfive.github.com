...
class FbConnectActions extends BaseFbConnectActions
{
  ...
  public function executeIndex(sfWebRequest $request)
  {
    ...
    $user = sfFbConnectGuardUserPeer::getSfGuardUserByFbId( $fb->user );
    ...
    if( $this->getUser()->isAuthenticated() ){
      ...
      sfFbConnectGuardUserPeer::linkSfFbUser( $fb->user, $sfUser->getUserId() );
    ...
    $sfGuardProfilePeerClass = sfConfig::get("app_sf_guard_plugin_profile_class", "SfGuardUserProfilePeer") . "Peer";
    $userCreateMethod = sfConfig::get("app_sf_fb_connect_guard_create_method", "createFbUser");
    $createFunction = "$sfGuardProfilePeerClass::$userCreateMethod";
    
    // see if we can call the custom create function
    if( method_exists($sfGuardProfilePeerClass, $userCreateMethod) ){
      $userId = call_user_func( $createFunction, $fb->user, $user_info );
    }else{
      // lets just use ours instead
      $userId = sfFbConnectGuardUserPeer::createFbUser( $fb->user );
    }
    
    // link em up
    sfFbConnectGuardUserPeer::linkSfFbUser( $fb->user, $userId );
